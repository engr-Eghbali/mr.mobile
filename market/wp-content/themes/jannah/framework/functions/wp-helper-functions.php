 ...
 */
function _jannah_good_get_category_by_slug( $slug ){
	return _jannah_good_get_term_by( 'slug', $slug, 'category' );
}
 ...
 * @link http://vip.wordpress.com/documentation/uncached-functions/ Uncached Functions
 */
function _jannah_good_get_term_by( $field, $value, $taxonomy, $output = OBJECT, $filter = 'raw' ){
...
/**
 * Properly clear _jannah_good_get_term_by() cache when a term is updated
 */
...
function _jannah_good_term_exists( $term, $taxonomy = '', $parent = null ){
...
/**
 * Properly clear _jannah_good_term_exists() cache when a term is updated
 */
 ...
 */
function _jannah_good_get_term_link( $term, $taxonomy = null ){
	...
	$term_object = _jannah_good_get_term_by( 'slug', $term, $taxonomy );
 ...
 * @link http://vip.wordpress.com/documentation/uncached-functions/ Uncached Functions
 */
function _jannah_good_get_page_by_title( $title, $output = OBJECT, $post_type = 'page' ){
 ...
 * @link http://vip.wordpress.com/documentation/uncached-functions/ Uncached Functions
 */
function _jannah_good_get_page_by_path( $page_path, $output = OBJECT, $post_type = 'page' ){
 ...
 * @link http://vip.wordpress.com/documentation/uncached-functions/ Uncached Functions
 */
function _jannah_good_flush_get_page_by_title_cache( $new_status, $old_status, $post ){
	if ( 'publish' == $new_status || 'publish' == $old_status )
		wp_cache_delete( $post->post_type . '_' . sanitize_key( $post->post_title ), 'get_page_by_title' );
}
add_action( 'transition_post_status', '_jannah_good_flush_get_page_by_title_cache', 10, 3 );
 ...
 * @link http://vip.wordpress.com/documentation/uncached-functions/ Uncached Functions
 */
function _jannah_good_flush_get_page_by_path_cache( $new_status, $old_status, $post ){
	if ( 'publish' === $new_status || 'publish' === $old_status ){
		$page_path = get_page_uri( $post->ID );
		wp_cache_delete( $post->post_type . '_' . sanitize_key( $page_path ), 'get_page_by_path' );
	}
}
add_action( 'transition_post_status', '_jannah_good_flush_get_page_by_path_cache', 10, 3 );
 ...
 */
function _jannah_good_url_to_postid( $url ){
	...
	if ( ! did_action( 'init' ) ){
		_doing_it_wrong( '_jannah_good_url_to_postid', '_jannah_good_url_to_postid must be called after the init action, as home_url has not yet been filtered', '' );

		return 0;
	}

	// Sanity check; no URLs not from this site
	if ( parse_url( $url, PHP_URL_HOST ) != _jannah_good_get_home_host() )
		return 0;
 ...
 * @deprecated use _jannah_good_wp_old_slug_redirect instead
 */
function _jannah_good_old_slug_redirect(){
    global $wp_query;
 ...
 * the cache
 */
function _jannah_good_count_user_posts( $user_id ){
 ...
 * This function helps prevent that by taking advantage of _jannah_good_get_term_by function which adds a layer of caching.
 *
 * @param string $menu Menu ID, slug, or name.
 * @uses _jannah_good_get_term_by
 * @return mixed false if $menu param isn't supplied or term does not exist, menu object if successful.
 */
function _jannah_good_get_nav_menu_object( $menu ){
	if ( ! $menu )
		return false;
	...
	if ( ! $menu_obj ){
		$menu_obj = _jannah_good_get_term_by( 'slug', $menu, 'nav_menu' );
	}

	if ( ! $menu_obj ){
		$menu_obj = _jannah_good_get_term_by( 'name', $menu, 'nav_menu' );
	}
 ...
 */
function _jannah_good_cache_set( $key, $value, $group = '', $expiration = 0 ){
 ...
 */
function _jannah_good_cache_get( $key, $group = '' ){
	if( ! class_exists( 'Stampedeless_Cache' ) )
 ...
 */
function _jannah_good_cache_delete( $key, $group = '' ){
	//delete cache itself
 ...
 */
function _jannah_good_get_adjacent_post( $in_same_term = false, $excluded_terms = '', $previous = true, $taxonomy = 'category', $adjacent = '' ){
	global $wpdb;
	...
	if ( $in_same_term ){
		if ( is_object_in_taxonomy( $post->post_type, $taxonomy ) ){
			...
			if ( ! empty( $term_array ) && ! is_wp_error( $term_array ) ){
				...
				$term_id_to_search = apply_filters( '_jannah_good_limit_adjacent_post_term_id', $term_id_to_search, $term_array_ids, $excluded_terms, $taxonomy, $previous );
	...
	$query_key = '_jannah_good_adjacent_post_' . md5( $query );
...
function _jannah_good_attachment_url_to_postid( $url ){

	$id = wp_cache_get( "_jannah_good_attachment_url_post_id_". md5( $url ) );
	if ( false === $id ){
		$id = attachment_url_to_postid( $url );
		if ( empty( $id ) ){
			wp_cache_set( "_jannah_good_attachment_url_post_id_". md5( $url ) , 'not_found', 'default', 12 * HOUR_IN_SECONDS + mt_rand(0, 4 * HOUR_IN_SECONDS ) );
		}else {
			wp_cache_set( "_jannah_good_attachment_url_post_id_". md5( $url ) , $id, 'default', 24 * HOUR_IN_SECONDS + mt_rand(0, 12 * HOUR_IN_SECONDS ) );
		}
 ...
 */
function _jannah_good_cache_full_comment_counts( $counts = false , $post_id = 0 ){
...
function _jannah_good_enable_cache_full_comment_counts(){
	add_filter( 'wp_count_comments', '_jannah_good_cache_full_comment_counts', 10, 2 );
}


function _jannah_good_enable_old_slug_redirect_caching(){
	add_action('template_redirect', '_jannah_good_wp_old_slug_redirect', 8 );
}

function _jannah_good_wp_old_slug_redirect(){
	global $wp_query;
	if ( is_404() && '' !== $wp_query->query_vars['name'] ){
		...
		if ( false === $redirect ){
			add_filter('old_slug_redirect_url', '_jannah_good_set_old_slug_redirect_cache');
			//If an old slug is not found the function returns early and does not apply the old_slug_redirect_url filter. so we will set the cache for not found and if it is found it will be overwritten later in _jannah_good_set_old_slug_redirect_cache()
			wp_cache_set( 'old_slug'. $wp_query->query_vars['name'], 'not_found', 'default', 4 * HOUR_IN_SECONDS );
		} elseif ( 'not_found' === $redirect ){
			//_jannah_good_set_old_slug_redirect_cache() will cache 'not_found' when a url is not found so we don't keep hammering the database
...
function _jannah_good_set_old_slug_redirect_cache( $link ){
	global $wp_query;
	if ( ! empty( $link ) ){
